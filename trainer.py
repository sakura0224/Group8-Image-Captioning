...
def evaluate(subset, encoder, decoder, config, device):
    ...
    # 使用 tqdm 进度条
    with tqdm(total=len(subset) // batch_size, desc="Evaluating", unit="batch") as pbar:
    ...
    bleu_1 = corpus_bleu(references_total, predictions_total,
                         weights=bleu_w["bleu-1"], smoothing_function=smoothing_function) * 100
    bleu_2 = corpus_bleu(references_total, predictions_total,
                         weights=bleu_w["bleu-2"], smoothing_function=smoothing_function) * 100
    bleu_3 = corpus_bleu(references_total, predictions_total,
                         weights=bleu_w["bleu-3"], smoothing_function=smoothing_function) * 100
    bleu_4 = corpus_bleu(references_total, predictions_total,
                         weights=bleu_w["bleu-4"], smoothing_function=smoothing_function) * 100
...
def train(config, writer, device):
    ...
    train_set = Flickr8KDataset(
        config, config["split_save"]["train"], training=True)
    ...
    encoder = models.resnet50(weights=ResNet50_Weights.IMAGENET1K_V1)
    ...
    for epoch in range(train_config["num_of_epochs"]):
        ...
        # 使用 tqdm 进度条
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{train_config['num_of_epochs']}", unit="batch") as pbar:
            for x_img, x_words, y, tgt_padding_mask in train_loader:
                ...
                torch.nn.utils.clip_grad_norm_(
                    decoder.parameters(), train_config["gradient_clipping"])
        ...
        if (epoch + 1) % train_config["eval_period"] == 0:
            with torch.no_grad():
                ...
                train_bleu = evaluate(train_set, encoder,
                                      decoder, config, device)
                valid_bleu = evaluate(valid_set, encoder,
                                      decoder, config, device)
