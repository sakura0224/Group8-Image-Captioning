...
def greedy_decoding(model, img_features_batched, sos_id, eos_id, pad_id, idx2word, max_len, device):
    ...
    x_words = torch.Tensor([sos_id] + [pad_id] *
                           (max_len - 1)).to(device).long()
    ...
    for i in range(max_len - 1):
        ...
        y_pred_prob = y_pred_prob[torch.arange(
            batch_size), [i] * batch_size].clone()
        ...
        for batch_idx in range(batch_size):
            if is_decoded[batch_idx]:
                continue
            # Add the generated word to the caption //generated_captions[batch_idx].append(idx2word[str(y_pred[batch_idx].item())])
            generated_captions[batch_idx].append(
                idx2word[y_pred[batch_idx].item()])
        ...
        if i < (max_len - 1):   # We haven't reached maximum number of decoding steps
            ...
            x_words[torch.arange(batch_size), [i+1] *
                    batch_size] = y_pred.view(-1)
